...
export const operations: { [opcode: string]: Operation | DynamicOp } = {
  ...
  ADD: (state: MachineState): MachineState => {
    let fst: N256;
    [fst, state] = state.popStack();
    let snd: N256;
    [snd, state] = state.popStack();
  ...
  SIGNEXTEND: (state: MachineState): MachineState => {
    let fst: N256;
    [fst, state] = state.popStack();
    let snd: N256;
    [snd, state] = state.popStack();
    return state.pushStack(snd.signExtend(fst))
      .appendLogInfo(`(${fst.toNumber()}, ${snd.toNumber()})`);
  },
  ...
  CODECOPY: (state: MachineState): MachineState => {
    ...
    let memOffset: N256;
    [memOffset, state] = state.popStack();
    let codeOffset: N256;
    [codeOffset, state] = state.popStack();
    let length: N256;
    [length, state] = state.popStack();
...
${state.code.slice(codeOffset.toNumber(), codeOffset.add(length).toNumber()).toString('hex')}`);
  ...
  LOG: (param: number): Operation => (state: MachineState): MachineState => {
    // TODO
    return state;
  },
